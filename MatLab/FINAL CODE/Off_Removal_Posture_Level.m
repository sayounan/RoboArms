...
for j=1:11
    if j == 4
        Sum1(j,:) = sum(abs(EMGfilt.(Names{j}).posture.Data))-abs(EMGfilt.(Names{j}).posture.Data(1,:))-abs(EMGfilt.(Names{j}).posture.Data(3,:))-abs(EMGfilt.(Names{j}).posture.Data(5,:))-abs(EMGfilt.(Names{j}).posture.Data(7,:));
        ...
        % for posture level off/spike removal.
    elseif j== 10
        Sum1(j,:) = sum(abs(EMGfilt.(Names{j}).posture.Data))-abs(EMGfilt.(Names{j}).posture.Data(3,:));
        % Posture 10 has an antagonist that spikes at the end of each "on"
        % period, which gives each trial a trailing end I don't want.
    else
...
for k=2:11
    ...
    Smoothed = smoothdata(Sum1(k,:),'sgolay',(SmoothBINsize*1000/Fs));
...
% threshold for spike removal
       ...
       if Smoothed(1,i) > Median + 2.5*Std
            OnOffIndex(k,i)=0;
       elseif Smoothed(1,i) > Median + .75*Std
           OnOffIndex(k,i)=1;
       else
            OnOffIndex(k,i)=0;
       end
